...
document.getElementById('btnDownloadDesktop').addEventListener('click', () => {
  exportWallpaper(DESKTOP_W, DESKTOP_H, wp.pattern, wp.palette, wp.seed, wp.inverted, `pixelcraft-${wp.id}-desktop.png`);
  dropdown.classList.remove('open');
});

document.getElementById('btnDownloadMobile').addEventListener('click', () => {
  exportWallpaper(MOBILE_W, MOBILE_H, wp.pattern, wp.palette, wp.seed, wp.inverted, `pixelcraft-${wp.id}-mobile.png`);
  dropdown.classList.remove('open');
});
...
document.getElementById('btnSetWallpaper').addEventListener('click', async () => {
  try {
    ...
    if (window.pywebview && window.pywebview.api) {
      await window.pywebview.api.set_wallpaper(bytes, `pixelcraft-${wp.id}.png`);
    } else if (window.__TAURI__) {
      await window.__TAURI__.core.invoke('set_wallpaper', {
        bytes,
        filename: `pixelcraft-${wp.id}.png`,
      });
    }
